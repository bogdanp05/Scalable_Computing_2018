...
class FmAlgo {
  ...
  def removeDirectory(dir:File){
    try {
      if (dir.isDirectory) {
        val files = dir.listFiles
        if (files != null && files.length > 0) for (aFile <- files) {
...
//    }.cache
    ...
    val zip = "myCollaborativeFilter.zip"
    println("====44====")

    println(model.userFeatures.first())
    println(model.productFeatures.first().getClass)
    println(model.rank)

    val dir = new File(path + src)
    removeDirectory(dir)
    model.save(sc, path + src)
    MakeZipUnZip.zipDirectory(dir, new File(path + zip))

    val loaded = MatrixFactorizationModel.load(sc, path + src)
    println(loaded.rank)

    val W: BDM[Double] = null
    return W
...
object userConverter {
  ...
  def convertIdToInt(rddToConvert: RDD[Row]): Map[String, Int] = {
    ...
    val reverseMapping1: RDD[(Long, String)] =
...
object songConverter{
  def convertIdToInt(rddToConvert: RDD[Row]): Map[String, Int] = {
    ...
    val songIdToInt: RDD[(String, Long)] =
      rddToConvert.map(_.getAs[String]("song")).distinct().zipWithUniqueId()

    val reverseMapping2: RDD[(Long, String)] =
...
object ratingCreater{
  def createRatings (rddToConvert: RDD[Row]): RDD[Rating] = {
    ...
    val ratings: RDD[Rating] = rddToConvert.map { obj =>
